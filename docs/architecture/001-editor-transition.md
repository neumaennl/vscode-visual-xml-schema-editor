...
- [x] Implement annotation and documentation commands (add, remove, modify)
...
- [x] Harden command validation and error reporting — all validators return `{ valid, error }` (no thrown exceptions); `CommandExecutionResult.errorKind` distinguishes validation vs. runtime failures; `SchemaEditorProvider` routes them to `commandResult` vs. `error` messages respectively
