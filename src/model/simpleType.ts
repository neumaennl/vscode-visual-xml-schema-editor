...
export class XsdSimpleType extends VisualXsdComponent {
  ...
  static fromJSON(json: SerializedNode): XsdSimpleType {
    ...
    return new XsdSimpleType(
      json.name,
      json.baseType,
      json.restrictions?.map((r: SerializedNode) =>
        XsdRestriction.fromJSON(r)
      ) ?? [],
      json.isList ?? false
    );
  }
  ...
  toXsd(): string {
    const restrictionContent = this.restrictions
      .map((r) => r.toXsd())
      .join("\n");
...
export class XsdRestriction extends VisualXsdComponent {
  constructor(public type: RestrictionType, public value: string) {
    super();
  }
  ...
  static fromJSON(json: SerializedNode): XsdRestriction {
    ...
    return new XsdRestriction(json.type, json.value);
  }
