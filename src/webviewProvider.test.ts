...
const mockGetUri = vi.fn((path: vscode.Uri): vscode.Uri => path);
...
describe("SchemaEditorProvider", () => {
  ...
  beforeEach(() => {
    ...
    mockWebview = {
      ...
      postMessage: mockPostMessage,
        onDidReceiveMessage: vi.fn<(listener: (message: unknown) => void) => vscode.Disposable>(),
  ...
  describe("executeCommand error routing", () => {
    // Use vi.spyOn on the CommandProcessor prototype so we can control the
